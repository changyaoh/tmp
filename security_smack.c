			     ...
			     void *opaque)
{
    ...
    return SmackRestoreSecurityFileLabel(file,opaque);
}
...
static int
SmackSetFileLabelHelper(const char *path, char *tlabel)
{
   char * elabel;
...
static int
SmackSetFileLabel(const char *path,char *tlabel)
{
   return SmackSetFileLabelHelper(const char *path, char *tlabel);
}


static int
SmackRestoreSecurityFileLabel(virSecurityManagerPtr mgr,
		              const char *path)
{
      struct stat buf;
      int ret = -1;
...
static int
SmackSecurityDriverProbe(const char *virDriver)
{
...
SmackSetSecurityDaemonSocketLabel(virSecurityManagerPtr mgr ATTRIBUTE_UNUSED, virDomainDefPtr vm)
{
    ...
    VIR_DEBUG("Setting VM %s socket label %s",def->name,label);
...
static int
SmackSetTapFDLabel(virSecurityManagerPtr mgr,
	           virDomainDefPtr def,
                   int fd) 
{
    struct stat buf;
...
static char *
SmackGetMountOptions(virSecurityManagerPtr mgr,
		     virDomainDefPtr def)
{
	char *opts = NULL;
	virSecurityLabelDefPtr secdef;
